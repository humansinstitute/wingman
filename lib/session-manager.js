      ...
      await new Promise((resolve, reject) => {
        deleteProcess.on('close', (code) => {
          if (code === 0) {
            ...
            // Don't fail if goose deletion fails - continue with cleanup
            console.warn(`Goose session deletion returned code ${code}`);
            resolve();
          }
        });
          ...
          if (code === 0) {
            try {
              ...
              const mergedSessions = gooseSessions.map(session => ({
                ...session,
                isRunning: runningSessionNames.has(session.name),
                sessionId: runningSessions.find(s => s.sessionName === session.name)?.sessionId
              }));
