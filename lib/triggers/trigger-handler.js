class TriggerHandler {
  ...
  async createTriggeredSession(recipe, prompt, customSessionName, workingDir) {
    ...
    // Determine working directory - expand ~ if present, fallback to current directory
    let resolvedWorkingDir = process.cwd();
    if (workingDir) {
      if (workingDir.startsWith('~/')) {
        const os = require('os');
        resolvedWorkingDir = workingDir.replace('~', os.homedir());
      } else {
  ...
  async processTrigger(requestData, token) {
    ...
    try {
      ...
      const { recipe_id, prompt, session_name, dir } = requestData;
      ...
      const sessionInfo = await this.createTriggeredSession(recipe, prompt, session_name, dir);
