...
class DatabaseManager {
  ...
  async deleteOldArchivedSessions(days = 30) {
    ...
    const result = await this.run(
      'DELETE FROM sessions WHERE archived = 1 AND archived_at < ?',
      [cutoffISOString]
    );
    return result.changes;
  }
  ...
  async getOldArchivedSessionsCount(days = 30) {
    ...
    const result = await this.get(
      'SELECT COUNT(*) as count FROM sessions WHERE archived = 1 AND archived_at < ?',
      [cutoffISOString]
    );
    return result.count;
  }
  ...
  async getOldArchivedSessions(days = 30) {
    ...
    return await this.all(
      'SELECT * FROM sessions WHERE archived = 1 AND archived_at < ?',
      [cutoffISOString]
    );
  }
